...
const getCourseKeys = (obj) => {
  const X = Object.keys(courseInfo);
  return X;
};
...
const hasChildrenValues = (arr, character) => {
  const children = Object.values(arr);
  let result;
  children.forEach(adult => {
    if (adult.name === character || adult.spouse === character) {
      const childs = adult.children;
      if (childs.length > 0) {
        result = true;
      } else result = false;
...
const hasChildrenEntries = (arr, character) => {
  const t = Object.entries(arr);
  let answer;
  t.forEach(adult => {
    if (adult[1].name === character || adult[1].spouse === character) {
      const childs = adult[1].children;
      if (childs.length > 0) {
        answer = true;
      } else answer = false;
    }
...
const totalCharacters = (arr) => {
  ...
  result.forEach(joker => {
    if(joker.name !==null) yo++;
    if(joker.spouse !==null) yo++;
    for(let person in joker.children){
      yo++;
    }
  })
  return yo;
